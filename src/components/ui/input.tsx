...
export type InputProps = Omit<React.ComponentProps<"input">, "className"> & {
  ...
  itemProps?: {
    ...
    description?: React.ComponentProps<"small">;
  };
};
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      >
        ...
        <input
          ...
          className={cn(
            ...
            !hasError && "focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]"
          )}
          autoComplete="off"
      ...
      {error && (
        <div
          {...itemProps?.error}
          className={cn("text-sm font-medium text-destructive py-1 indent-2", itemProps?.error?.className)}
        >
          {error}
        </div>
      )}
      {description && !error && (
        <small
          ...
          className={cn("text-sm text-muted-foreground py-1 indent-2", itemProps?.description?.className)}
        >
          {description}
        </small>
      )}
