...
export const createClassGroupUtils = (config: AnyConfig) => {
    ...
    const getClassGroupId = (className: string) => {
        ...
        // class like `-inset-1` produce an empty string as first classPart. We assume that class for negative values are used correctly and skip it.
...
const processClassGroups = (
    ...
    for (const classGroupId in classGroups) {
        ...
        processclassRecursively(group, classMap, classGroupId, theme)
    }

    return classMap
}

const processclassRecursively = (
...
const processClassDefinition = (
    ...
    if (typeof classDefinition === 'function') {
        processFunctionDefinition(classDefinition as ThemeGetter, classPartObject, classGroupId, theme)
        return
    }
...
const processFunctionDefinition = (
    classDefinition: ThemeGetter,
    classPartObject: ClassPartObject,
    classGroupId: AnyClassGroupIds,
    theme: ThemeObject<AnyThemeGroupIds>,
) => {
    if (isThemeGetter(classDefinition)) {
        processclassRecursively(classDefinition(theme), classPartObject, classGroupId, theme)
        return
    }
...
const processObjectDefinition = (
    ...
    for (let i = 0; i < len; i++) {
        ...
        processclassRecursively(value, getPart(classPartObject, key), classGroupId, theme)
    }
}
...
const isThemeGetter = (func: ThemeGetter): func is ThemeGetter =>
