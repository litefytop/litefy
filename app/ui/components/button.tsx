...
const buttonClassBase =
  "cursor-pointer inline-flex items-center justify-center gap-2 h-8 min-w-8 px-3 rounded-md text-sm text-center shadow-base focus-visible:outline-1 focus-visible:outline-outline focus-visible:ring-3 focus-visible:ring-ring/50 active:shadow-[inset_0_2px_4px_0_var(--accent)] [&_svg:not([class*='size-'])]:size-4";

const buttonClassVariant = {
...
export interface ButtonProps extends Omit<React.ComponentProps<"button">, "className"> {
  variant?: keyof typeof buttonClassVariant;
...
Button.className = {
  primary: cn(buttonClassBase, buttonClassVariant.primary),
  danger: cn(buttonClassBase, buttonClassVariant.danger),
  outline: cn(buttonClassBase, buttonClassVariant.outline),
  text: cn(buttonClassBase, buttonClassVariant.text),
};
