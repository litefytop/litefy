...
function Sidebar({ ref, children, className, defaultOpen = true, ...props }: SidebarProps) {
  ...
  return (
    <aside
      ...
      className={cn(
        className,
        "h-full overflow-hidden bg-background text-foreground transition-[width,padding,margin] duration-300 ease-in-out data-close:w-0 data-close:p-0 data-close:m-0",
      )}
    >
      {children}
    </aside>
