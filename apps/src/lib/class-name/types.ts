...
interface ConfigStaticPart {
    ...
    cacheSize: number
    /**
     * Prefix added to Tailwind-generated class
     * @see https://tailwindcss.com/docs/configuration#prefix
     */
    prefix?: string
    /**
     * Allows to customize parsing of individual class passed to `twMerge`.
     * All class passed to `twMerge` outside of cache hits are passed to this function before it is determined whether the class is a valid Tailwind CSS class.
     *
...
export interface ParsedClassName {
     ...
     *
     * This property is prefixed with "maybe" because tailwind-merge does not know whether something is a postfix modifier or part of the base class since it's possible to configure Tailwind CSS class which include a `/` in the base class name.
     *
...
interface ConfigGroupsPart<ClassGroupIds extends string, ThemeGroupIds extends string> {
    ...
    /**
     * Object with groups of class.
     *
     * @example
     * {
     *     // Creates group of class `group`, `of` and `class`
     *     'group-id': ['group', 'of', 'class'],
     *     // Creates group of class `look-at-me-other` and `look-at-me-group`.
     *     'other-group': [{ 'look-at-me': ['other', 'group']}]
     * }
     */
    classGroups: NoInfer<Record<ClassGroupIds, ClassGroup<ThemeGroupIds>>>
    /**
     * Conflicting class across groups.
     *
     * The key is the ID of a class group which creates a conflict, values are IDs of class groups which receive a conflict. That means if a class from from the key ID is present, all preceding class from the values are removed.
     *
     ...
     *
     * tailwind-merge makes sure that class with these modifiers are not overwritten by class with the same modifiers with order-sensitive modifiers being in a different position.
     */
...
type PartialPartial<T> = {
    [P in keyof T]?: T[P] extends unknown[] ? T[P] : Partial<T[P]>
}
