...
| `placement`       | `'left' \| 'right' \| 'top' \| 'bottom'` | -       | Position of the drawer. **Required**                                                          |
...
| `classNames`      | `{ root?: ClassNameValue; wrapper?: ClassNameValue; drag?: ClassNameValue; content?: ClassNameValue }` | - | Custom classes for the internal elements |
| `styles`          | `{ root?: React.CSSProperties; wrapper?: React.CSSProperties; drag?: React.CSSProperties; content?: React.CSSProperties }` | - | Inline styles for the internal elements |
...
> The following components can be freely assembled. They carry the same default styles as the composite `Drawer`; override them via `className`. The composite's modal lifecycle (`showModal`, close animation, focus trap) is not part of the atoms — replicate it yourself when assembling.
...
| Prop           | Type                          | Default | Description                                        |
| -------------- | ----------------------------- | ------- | -------------------------------------------------- |
| `isHorizontal` | `boolean`                     | -       | Whether horizontal, centers content when true      |
| `className`    | `ClassNameValue`              | -       | Custom class name                                  |
