---
title: Query Builder
description: A collapsible folder tree of unique field nodes and nested and/or groups, with a live natural-language preview and Submit/Reset — zero external dependencies.
---
...
Fields are label/value pairs: `name` is what gets written into the query, `label` is what humans read — folder nodes and the preview always render the label, so the same query tree localizes freely.
...
| `label` | `string` | `name` | Field label shown on its folder node and in the preview |
