...
Operators are localized through the `QueryBuilder.operatorLabels` static — it feeds both the operator dropdowns and the preview. Unknown operators fall back to their raw string, so custom operators keep working without configuration.
