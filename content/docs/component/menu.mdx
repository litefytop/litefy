...
`Popover + Menu` builds a dropdown menu: use Popover for the trigger and the floating panel, put `Menu` inside the panel content, set `autoFocus={open}` so the first item is focused on each open, and pass `onEscape` to close the panel. See the [Dropdown Menu](/docs/component/dropdown-menu) guide for the complete assembly.
