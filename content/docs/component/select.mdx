...
| `invalid` | `boolean` | - | Applies the danger border / ring via `aria-invalid` |
...
| `classNames` | `{ trigger?; panel?; label?; option? }` — each `ClassNameValue` | - | Custom classes for the trigger, panel, group labels and options |
| `styles` | `{ trigger?; panel? }` — each `React.CSSProperties` | - | Inline styles for the trigger and panel |
