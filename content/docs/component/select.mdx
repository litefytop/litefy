...
Select component with a danger Callout rendered below as the error message.
...
- The trigger is fluid: `w-full` capped at `max-w-120` so the label line stays readable in wide containers; pass `className` to override.
