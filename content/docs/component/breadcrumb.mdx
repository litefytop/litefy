...
The hover-menu demo shows how to attach a hover menu to an entry with `PopoverContent` — the panel renders inline with `popover="manual"` (the top layer escapes ancestor `overflow` clipping), anchors below its entry, opens on hover or focus, and closes 120ms after leaving both the entry and the panel:
