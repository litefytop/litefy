...
Compose a form from `Form` + [FormItem](/docs/component/form-item) + `Form.Submit`. Group controls without a wrapper (checkbox / radio groups) submit natively — give them a `name` and they join the payload.
...
All field controls (Input, Textarea, Select, Password, NumberInput, Picker, InputOtp) style themselves from the `aria-invalid` attribute — no extra props needed:
