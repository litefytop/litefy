...
The Upload component handles selection, drag and drop and local validation only — it performs no network requests and keeps no upload state. Pair it with the [`useUploadMonitor`](/docs/hooks/use-upload-monitor) hook to execute uploads and track progress.
...
| `invalid` | `boolean` | `false` | Sets `data-invalid` on the root and `aria-invalid` on the dropzone |
