...
| `title`           | `React.ReactNode`                                   | -            | Optional title. When provided, renders a header row — title on the left, the close button fixed on the right in flow — so long titles wrap instead of overlapping the button |
...
| `...props`        | `React.ComponentProps<"div">`                       | -            | Remaining props are passed through to the content area `DialogContent` (except `className` / `styles` / `title`) |
...
Close button with built-in styles, absolutely positioned in the top-right corner of the nearest positioned ancestor (i.e. the dialog element). For parts assembly only — the composite `Dialog` already renders and wires this button internally, so never put `DialogClose` inside `Dialog` children.
