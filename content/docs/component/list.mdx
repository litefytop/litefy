...
Rows are contiguous — no built-in dividers (add `divide-y divide-border` via `className` if you want them). The root applies `overflow-y-auto` for scrolling, with `outline-none` and no built-in focus ring. Keyboard: `ArrowUp` / `ArrowDown` move the highlight, `Enter` or `Space` selects it.
