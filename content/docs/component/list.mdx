...
Rows are contiguous — no built-in dividers (add `divide-y divide-border` via `className` if you want them). Scrolling is managed by a built-in [ScrollShadow](/docs/component/scroll-shadow) wrapper: the native scrollbar is hidden and top/bottom gradient shadows with directional arrows appear while more content is available in that direction, with `outline-none` on the focusable list itself and no built-in focus ring. Keyboard: `ArrowUp` / `ArrowDown` move the highlight, `Enter` or `Space` selects it.
