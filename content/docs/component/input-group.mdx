...
Set the `invalid` prop (or `data-invalid`) on the group — not `aria-invalid` styling on the input. The inner input still carries `aria-invalid` for accessibility, and its danger text color is driven by the group through `group-data-invalid`.
