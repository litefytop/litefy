...
The visible month is fully controlled: hold it in state and update it via `onVisibleMonthChange`. Selection supports both controlled (`value` + `onChange`) and uncontrolled (`defaultValue`) modes.
...
**Localization**: month names come from the mutable static `Calendar.calendarMonthLabels` (English abbreviations by default). It is process-global and cannot be scoped to a single instance — override it once at your app's entry point and every calendar picks it up from the next render, including the one inside [DatePicker](/docs/component/datepicker). Weekday abbreviations in the day grid are fixed English labels.
