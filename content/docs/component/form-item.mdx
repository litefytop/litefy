---
title: Form Item
description: A self-managing form field that renders a finished control per variant — label, description, input and error in a fixed vertical layout with built-in validation
---
...
`FormItem` renders a finished control per `variant` (`input`, `textarea`, `select`, `password`, `number-input`) with a fixed vertical layout: **label → description (optional) → input → error**. It registers with the surrounding `Form` so its value is collected on submit, shows validation errors in a danger line below the input, and works standalone too.
...
| `description`    | `React.ReactNode`                                                                                                                                                 | -          | Helper text rendered between the label and the input                                               |
