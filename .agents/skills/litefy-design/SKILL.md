...
运行 `node scripts/design-detect.mjs`（若存在）扫描：

- 业务代码（`app/` 排除 `app/ui`）中的硬编码色值与任意值阴影
- 语义四级之外的 `shadow-*` / 已清除的默认刻度
- 业务代码里的手写焦点样式
- Chip/Badge/Card/CardButton 误用模式的启发式提示

检测结果为问题清单，处理方式遵循当前模式（审计记录 / 开发修复）。
